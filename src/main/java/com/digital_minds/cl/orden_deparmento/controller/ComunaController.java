...
public class ComunaController {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<Comuna> actualizarParcial(@PathVariable Integer id, @RequestBody Comuna parcialComuna) {
        Comuna comunaActualizada = comunaService.patchComuna(id, parcialComuna);
        if (comunaActualizada != null){
            return ResponseEntity.ok(comunaActualizada);
        } else {
            return ResponseEntity.notFound().build();
